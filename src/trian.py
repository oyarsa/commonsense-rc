...
class TriAN(nn.Module):
    def __init__(self, args):
        ...
        # Input size to RNN: word emb + question emb + pos emb + ner emb +
        # manual features
        doc_input_size = 2 * self.embedding_dim + args.pos_emb_dim + \
            args.ner_emb_dim + 5 + 2 * args.rel_emb_dim
        ...
        self.p_q_attn = layers.BilinearSeqAttn(
            x_size=doc_hidden_size, y_size=question_hidden_size)
    ...
    def forward(self, p, p_pos, p_ner, p_mask, q, q_pos, q_mask, c, c_mask,
                f_tensor, p_q_relation, p_c_relation):

        p_emb, q_emb, c_emb = (self.embedding(p),
                               self.embedding(q), self.embedding(c))
        p_pos_emb, p_ner_emb, q_pos_emb = (
            self.pos_embedding(p_pos), self.ner_embedding(p_ner),
            self.pos_embedding(q_pos))
        p_q_rel_emb, p_c_rel_emb = (
            self.rel_embedding(p_q_relation), self.rel_embedding(p_c_relation))

        # Dropout on embeddings
        if self.args.dropout_emb > 0:
            p_emb = nn.functional.dropout(
                p_emb, p=self.args.dropout_emb, training=self.training)
            q_emb = nn.functional.dropout(
                q_emb, p=self.args.dropout_emb, training=self.training)
            c_emb = nn.functional.dropout(
                c_emb, p=self.args.dropout_emb, training=self.training)
            p_pos_emb = nn.functional.dropout(
                p_pos_emb, p=self.args.dropout_emb, training=self.training)
            p_ner_emb = nn.functional.dropout(
                p_ner_emb, p=self.args.dropout_emb, training=self.training)
            q_pos_emb = nn.functional.dropout(
                q_pos_emb, p=self.args.dropout_emb, training=self.training)
            p_q_rel_emb = nn.functional.dropout(
                p_q_rel_emb, p=self.args.dropout_emb, training=self.training)
            p_c_rel_emb = nn.functional.dropout(
                p_c_rel_emb, p=self.args.dropout_emb, training=self.training)
        ...
        p_q_weighted_emb = nn.functional.dropout(
            p_q_weighted_emb, p=self.args.dropout_emb, training=self.training)
        c_q_weighted_emb = nn.functional.dropout(
            c_q_weighted_emb, p=self.args.dropout_emb, training=self.training)
        c_p_weighted_emb = nn.functional.dropout(
            c_p_weighted_emb, p=self.args.dropout_emb, training=self.training)

        p_rnn_input = torch.cat([p_emb, p_q_weighted_emb, p_pos_emb,
                                 p_ner_emb, f_tensor, p_q_rel_emb,
                                 p_c_rel_emb], dim=2)
        c_rnn_input = torch.cat(
            [c_emb, c_q_weighted_emb, c_p_weighted_emb], dim=2)
